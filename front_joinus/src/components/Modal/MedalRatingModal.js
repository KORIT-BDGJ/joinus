...
const MedalRatingModal = ({ modalState, postId, currentUserId, refetchHostFinishList, onComplete }) => {
  ...
  const getAttendList = useQuery(["getAttendList"], async () => {
    ...
    const response = await axios.get(
      `https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/post/${postId}/attend/list`,
      option
    );
  ...
  const saveChanges = useMutation(
    async (starValues) => {
      ...
      const response = await axios.put(
        `https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/account/point/rating`,
        starValues,
        option
      );
      return response.data;
        ...
        if (data) {
          ...
          await Promise.all(
            evaluatedUserIds.map((userId) =>
              axios.delete(`https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/post/${postId}/attend/delete`, {
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
                },
                params: {
                    ...
                    <div css={attendInfo}>
                      <div css={infoImage}>
                        {attendData.image ? (
                          <img
                            css={imgIcon}
                            src={`https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/image/profile/${attendData.image}`}
                            alt="ProfileImage"
                          />
                        ) : (
