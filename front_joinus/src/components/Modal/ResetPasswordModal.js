...
const ResetPasswordModal = ({ setIsOpen , setResetSuccess }) => {
  ...
  const checkToken = useQuery(["checkToken"], async () => {
    ...
    try {
      const response = await axios.get("https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/auth/forget/password/token", option);
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        'https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/auth/reset/password',
        {
