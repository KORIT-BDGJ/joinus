...
const Comment = ({ postId }) => {
    ...
    const getComment= useQuery(["getComment"], async () => {
        ...
        const response = await axios.get(`https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/post/${postId}/comment`, option);
        return response;
    });
    ...
    const commentSubmit = useMutation(async (postId) => {
        ...
        return await axios.post(`https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/post/${postId}/comment/submit`, JSON.stringify({
    ...
    const commentDelete = useMutation(async ({ postId, deleteCommentId }) => {
        ...
        return await axios.delete(`https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/post/${postId}/comment/delete`, option);
    }, {
        onSuccess: () => {
            queryClient.invalidateQueries("getComment");
        }
                        ...
                        <div css={chatting} key={commentData.commentId}>
                            <div css={chattingHeader}>
                                ...
                                {commentData.image ? (
                                        <img
                                            css={imgIcon}
                                            src={"https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/image/profile/" + commentData.image}
                                            alt="ProfileImage"
                                        />
                                    ) : (
