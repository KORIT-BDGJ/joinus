...
const ForgetPassword = () => {
    ...
    const checkEmailSubmitHandle = async() => {
        try {
            const response = await axios.put("https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/auth/forget/password", { email: loginUser.email });
          
            if (response.status === 200) {
                alert('일치하는 이메일 정보가 있습니다.');
                const sendResponse = await axios.post("https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/auth/validation/send", { email: loginUser.email });
