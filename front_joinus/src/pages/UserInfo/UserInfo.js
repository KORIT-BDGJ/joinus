...
const UserInfo = () => {
  ...
  useEffect(() => {
    ...
    if (principal.data) {
      ...
      setProfileImgURL("https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/image/profile/" + principal.data.image);
    }
  }, [principal.data]);  
  ...
  const profileImgSubmit = useMutation(async(selectedFile) => {
    ...
    const response = await axios.post("https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/account/profile/img", formData, option);
    return response;
  ...
  const handleModifyClick = async () => {
    ...
    try {
        const response = await axios.put("https://port-0-joinus-koh2xlitnedv8.sel4.cloudtype.app/account/change/sportslikes", 
            {
